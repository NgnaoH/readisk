...
_electron.ipcMain.on("close-app", function (event) {
  _electron.app.exit();
});

_electron.ipcMain.on("minimize-app", function (event) {
  win.minimize();
...
_electron.ipcMain.on("fetch-disks", function (event) {
  _systeminformation["default"].diskLayout(function (data) {
    event.reply("disk-layout", data);
  });

  _systeminformation["default"].blockDevices(function (data) {
